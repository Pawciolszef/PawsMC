...
const currentRgb = computed(() => {
	return hexToRgb(currentCustomHex.value) || { r: 236, g: 72, b: 153 }
})

const redChannel = ref(currentRgb.value.r)
const greenChannel = ref(currentRgb.value.g)
const blueChannel = ref(currentRgb.value.b)
const hexInput = ref(currentCustomHex.value)
...
watch(
	...
	(newHex) => {
		if (newHex) {
			...
			if (rgb) {
				redChannel.value = rgb.r
				greenChannel.value = rgb.g
				blueChannel.value = rgb.b
...
function updateFromRgb() {
	const hex = rgbToHex(redChannel.value, greenChannel.value, blueChannel.value)
	hexInput.value = hex
...
function updateFromHex(raw: string) {
	let val = raw.trim()
	if (!val.startsWith('#')) {
		val = '#' + val
	}
...
function selectAccent(opt: AccentOption) {
	...
	if (opt.id === 'custom') {
		emit('update:customHex', hexInput.value || '#ec4899')
	}
}
	...
	<div class="mt-8 border-0 border-t border-solid border-divider pt-6 flex flex-col gap-5">
		...
		<div class="flex flex-col gap-1">
			<h2 class="m-0 text-xl font-semibold text-contrast flex items-center gap-2">
				<span>🎨 PawsMC Theme & Accent Customization</span>
			</h2>
			<p class="m-0 text-secondary text-sm">
				Choose a curated accent palette or build your own custom RGB color with live preview and save confirmation.
			</p>
		</div>
		...
		<!-- Custom RGB Studio Panel (Active when Custom is selected or always accessible) -->
		<div
		...
		>
			<div class="flex items-center justify-between flex-wrap gap-2">
				<div class="flex items-center gap-2">
					...
					<span class="font-semibold text-contrast text-base">Custom RGB Color Studio</span>
				</div>
				<div class="flex items-center gap-2">
				...
				<div class="md:col-span-2 flex flex-col gap-3">
					<div class="flex items-center gap-3">
						...
						<div class="flex-1 flex flex-col gap-1">
							<label class="text-xs font-medium text-secondary">HEX Color Code</label>
					...
					<div class="flex items-center gap-1.5 flex-wrap">
						...
						<button
							v-for="qp in quickCustomPresets"
							:key="qp.hex"
							type="button"
			...
			<div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-surface-4/40">
				...
				<div class="flex flex-col gap-1 bg-surface-3/50 p-2.5 rounded-xl border border-surface-4/30">
					<div class="flex justify-between items-center text-xs font-semibold">
						...
						<span class="font-mono text-contrast">{{ redChannel }}</span>
					</div>
					<input
						type="range"
						min="0"
						max="255"
						v-model.number="redChannel"
				...
				<div class="flex flex-col gap-1 bg-surface-3/50 p-2.5 rounded-xl border border-surface-4/30">
					<div class="flex justify-between items-center text-xs font-semibold">
						...
						<span class="font-mono text-contrast">{{ greenChannel }}</span>
					</div>
					<input
						type="range"
						min="0"
						max="255"
						v-model.number="greenChannel"
				...
				<div class="flex flex-col gap-1 bg-surface-3/50 p-2.5 rounded-xl border border-surface-4/30">
					<div class="flex justify-between items-center text-xs font-semibold">
						...
						<span class="font-mono text-contrast">{{ blueChannel }}</span>
					</div>
					<input
						type="range"
						min="0"
						max="255"
						v-model.number="blueChannel"
